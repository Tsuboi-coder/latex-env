...
この機能は macOS 専用です。ヒラギノを必要としない文書自体は、`hiragino-base` または `hiragino-slides` を読み込まなければ macOS 固有フォントに依存しません。`scripts/latexmk-docker` は macOS のフォントディレクトリが存在するときだけマウントするため、Linux / WSL でも同じスクリプトを使用できます。
...
Linux / WSL では `/System/Library/Fonts` のマウントを自動的に省略するため、ヒラギノを使わない文書は同じ補助スクリプトでコンパイルできます。ヒラギノ専用の `hiragino-base` と `hiragino-slides` は使用できません。代替フォントへのフォールバックを備えたスタイルを使うか、各OSで利用可能なフォントを指定した別のプリセットを用意してください。
